...
def gerar_numeros_aleatorios(n=10000, min_val=1, max_val=20000):
...
if __name__ == "__main__":
    ...
    print(f"Tempo sequencial: {end - start:.4f} segundos\n")

    # Com threads (versão paralela)
    start = time.time()
    numeros_ordenados_paralelo = quicksort(numeros.copy())
    end = time.time()
    print("Primeiros 10 números após a ordenação (paralelo):", numeros_ordenados_paralelo[:10])
    print(f"Tempo com threads: {end - start:.4f} segundos")
